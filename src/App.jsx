...
export default function App() {
  ...
  const [items, setItems] = useState(() => {
    // 优先从本地缓存加载
    const saved = localStorage.getItem('eqs_data_store');
    if (saved) {
      try {
   ...
   */
  const handleSave = () => {
    localStorage.setItem('eqs_data_store', JSON.stringify(items));
    showToast("数据已成功存档至浏览器本地");
  };
  ...
  const handleExport = () => {
    ...
    const headers = ["项目分类", "项目名称", "点位数量", "单位", "工程总量", "单价", "总价", "备注"];
    ...
    const rows = allProcessed.map(it => [
      ...
      it.unitPrice,
      it.amount,
      it.instruction.replace(/,/g, '，') // 避免CSV冲突
    ]);
    ...
    link.setAttribute("download", `工程量统计表_${new Date().toLocaleDateString()}.csv`);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        showToast("导入失败：请确保文件格式正确");
      }
    };
    // 也可以将当前的 items 存为 JSON 导出供人互换
    reader.readAsText(file);
    e.target.value = null; // 重置
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-32 text-xs sm:text-sm">
      ...
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="bg-slate-800 text-white px-4 py-2 rounded-full shadow-2xl flex items-center gap-2">
        ...
        <aside className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border p-4 space-y-4 sticky top-20">
            <div className="flex items-center gap-2 border-b pb-2 text-indigo-600">
              <RefreshCcw size={14} />
              <h2 className="font-black text-[10px] uppercase">全局频率</h2>
            </div>
            ...
            <div className="p-5 space-y-5">
              <div className="flex items-center justify-between p-3 bg-indigo-50 rounded-2xl border border-indigo-100">
                <div className="space-y-0.5">
                  <div className="font-bold text-[11px] text-indigo-900">打开独立数据</div>
